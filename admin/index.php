<?php
session_start();
include('includes/config.php');
include_once(__DIR__ . '/../includes/password-helper.php');
include_once(__DIR__ . '/../includes/validation-helper.php');
if(isset($_POST['login']))
{
$uname=dreamCleanInput($_POST['username']);
$password=$_POST['password'];
if(!dreamTextLength($uname, 3, 50) || $password === '') {
echo "<script>alert('Te dhenat e gabuara !');</script>";
} else {
$sql ="SELECT UserName,Password FROM admin WHERE UserName=:uname";
$query= $dbh -> prepare($sql);
$query-> bindParam(':uname', $uname, PDO::PARAM_STR);
$query-> execute();
$result=$query->fetch(PDO::FETCH_OBJ);
if($result && dreamPasswordMatches($password, $result->Password))
{
if(dreamPasswordNeedsUpgrade($result->Password)) {
$hashedPassword=dreamPasswordHash($password);
$updateSql="UPDATE admin SET Password=:password WHERE UserName=:uname";
$updateQuery=$dbh->prepare($updateSql);
$updateQuery->bindParam(':password',$hashedPassword,PDO::PARAM_STR);
$updateQuery->bindParam(':uname',$uname,PDO::PARAM_STR);
$updateQuery->execute();
}
$_SESSION['alogin']=$_POST['username'];
echo "<script type='text/javascript'> document.location = 'dashboard.php'; </script>";
} else{
	
	echo "<script>alert('Te dhenat e gabuara !');</script>";

}
}

}

?>

<!DOCTYPE HTML>
<html>
<head>
<title>Dream Travel | Logimi Admin </title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<script type="application/x-javascript"> addEventListener("load", function() { setTimeout(hideURLbar, 0); }, false); function hideURLbar(){ window.scrollTo(0,1); } </script>
<!-- Bootstrap Core CSS -->
<link href="css/bootstrap.min.css" rel='stylesheet' type='text/css' />
<!-- Custom CSS -->
<link href="css/style.css" rel='stylesheet' type='text/css' />
<link rel="stylesheet" href="css/morris.css" type="text/css"/>
<!-- Graph CSS -->
<link href="css/font-awesome.css" rel="stylesheet">
<link rel="stylesheet" href="css/jquery-ui.css"> 
<!-- jQuery -->
<script src="js/jquery-2.1.4.min.js"></script>
<!-- //jQuery -->
<link href='//fonts.googleapis.com/css?family=Roboto:700,500,300,100italic,100,400' rel='stylesheet' type='text/css'/>
<link href='//fonts.googleapis.com/css?family=Montserrat:400,700' rel='stylesheet' type='text/css'>
<link rel="stylesheet" href="css/icon-font.min.css" type='text/css' />

</head> 
<body>
	<div class="main-wthree">
	<div class="container">
	<div class="sin-w3-agile">
		<h2>Logohu</h2>
		<form  method="post">
			<div class="username">
				<span class="username">Username:</span>
				<input type="text" name="username" class="name" placeholder="" minlength="3" maxlength="50" required="">
				<div class="clearfix"></div>
			</div>
			<div class="password-agileits">
				<span class="username">Fjalekalimi:</span>
				<input type="password" name="password" class="password" placeholder="" minlength="6" maxlength="72" required="">
				<div class="clearfix"></div>
			</div>
			
			<div class="login-w3">
					<input type="submit" class="login" name="login" value="Logohu">
			</div>
			<div class="clearfix"></div>
		</form>
				<div class="back">
					<a href="../index.php">Kryefaqja </a>
				</div>
				
	</div>
	</div>
	</div>
</body>
</html>
