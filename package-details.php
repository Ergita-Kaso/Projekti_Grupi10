...
} else if(!preg_match('/^[0-9+()\-\s]{7,20}$/', $contactphone)) {
$error="Ju lutemi vendosni nje numer telefoni te vlefshem.";
} else if(strlen($pickup) < 3) {
...
<form name="book" method="post">
		<div class="selectroom_top">
			...
			<div class="col-md-8 selectroom_right wow fadeInRight animated" data-wow-delay=".5s">
				...
				<input class="date" id="datepicker" type="date" name="fromdate" min="<?php echo date('Y-m-d'); ?>" onclick="if(this.showPicker){this.showPicker();}" onkeydown="if((event.key==='Enter' || event.key===' ') && this.showPicker){event.preventDefault();this.showPicker();}" required="">
			</div>
			<div class="bnr-right">
				<label class="inputLabel">Ne:</label>
				<input class="date" id="datepicker1" type="date" name="todate" min="<?php echo date('Y-m-d'); ?>" onclick="if(this.showPicker){this.showPicker();}" onkeydown="if((event.key==='Enter' || event.key===' ') && this.showPicker){event.preventDefault();this.showPicker();}" required="">
			</div>
			</div>
			...
			<div class="selectroom-info animated wow fadeInUp animated" data-wow-duration="1200ms" data-wow-delay="500ms" style="visibility: visible; animation-duration: 1200ms; animation-delay: 500ms; animation-name: fadeInUp;">
				<ul>
					...
					<li class="nam">
						...
						<input type="tel" name="contactphone" pattern="[0-9+()\-\s]{7,20}" placeholder="p.sh. +355 69 123 4567" required="">
					</li>
					<li class="spe">
						<label class="inputLabel">Vendi i nisjes / marrjes</label>
						<input class="special" type="text" name="pickup" minlength="3" placeholder="Qyteti, hoteli ose adresa" required="">
					</li>
					<li class="spe">
						<label class="inputLabel">Komente</label>
						<input class="special" type="text" name="comment" placeholder="Kerkesa te vecanta per udhetimin">
					</li>
